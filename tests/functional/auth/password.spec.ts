...
test.group('Auth password', (group) => {
  ...
  test('should gracefully error if provided email does not exist', async ({ client }) => {
    const response = await client.post('/forgot-password').header('Referer', '/contact').form({
  ...
  //   const response = await client.post('/reset-password').header('Referer', '/contact').form({
