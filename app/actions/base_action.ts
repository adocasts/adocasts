...
export default abstract class BaseAction<HandleArgs extends any[] = []> implements BaseActionable {
  ...
  static async run<T extends BaseAction<any>>(
    this: StaticAction<T>,
    ...args: T extends { handle: (...args: infer P) => any } ? P : never
  ): Promise<T extends { handle: (...args: any) => infer R } ? R : never> {
  ...
  static async handleController<T extends BaseAction<HandleArgs>, HandleArgs extends any[] = []>(
    this: StaticAction<T>,
    ...args: any[]
  ) {
