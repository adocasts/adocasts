...
export default class RenderSitemapXml extends BaseAction {
  async asController({ response }: HttpContext) {
    let urls = await this.#getCachedSitemapUrls()
  ...
  async #getSitemapUrls() {
    ...
    // topics
    const rootTopics = await Taxonomy.roots()
      .apply((scope) => scope.hasContent())
      .orderBy('name', 'asc')
      .select('slug')
    const childTopics = await Taxonomy.children()
      .apply((scope) => scope.hasContent())
      .orderBy('name', 'asc')
      .select('slug')
    ...
    news.map(
      ({ slug }) => (urls = this.#add(urls, this.#map('news.show', { slug }, 'weekly', 0.3)))
    )
    streams.map(
      ({ slug }) => (urls = this.#add(urls, this.#map('streams.show', { slug }, 'weekly', 0.3)))
    )
    blogs.map(
      ({ slug }) => (urls = this.#add(urls, this.#map('blogs.show', { slug }, 'weekly', 0.2)))
    )

    return urls
  }

  #make(url: string, changefreq: Frequency, priority: number = 0.5): SiteMapItem {
  ...
  #add(urls: SiteMapItem[], conditionalAdd: SiteMapItem | undefined) {
