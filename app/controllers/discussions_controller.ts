...
import {
  discussionCreateValidator,
  discussionSearchValidator,
} from '#validators/discussion_validator'
...
export default class DiscussionsController {
  ...
  async search({ view, request, up }: HttpContext) {
  ...
  @inject()
  async show({ view, params, auth }: HttpContext, sessionService: SessionService) {
  ...
  async update({ request, response, params, session, auth, bouncer }: HttpContext) {
  ...
  @inject()
  async impression({ response, params, auth }: HttpContext, sessionService: SessionService) {
    const item = await Discussion.query()
      .where('id', params.id)
      .preload('impressions', (query) =>
        query
