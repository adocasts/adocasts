...
export default class NotificationListener {
  async onSend({ notification, user }: { notification: Notification; user: User }) {
    const domain = env.get('APP_DOMAIN')
    const href = domain + notification.href

    const turnOffFieldHref =
      domain +
      router.makeSignedUrl('users.notifications.disable.field', {
        userId: user.id,
    ...
    const turnOffHref =
      domain +
      router.makeSignedUrl('users.notifications.disable', {
        userId: user.id,
      })
