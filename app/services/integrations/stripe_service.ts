import RemoveUserFromGitHubTeam from '#actions/integrations/github/remove_user_from_github_team'
import SendGitHubTeamInvite from '#actions/integrations/github/send_github_team_invite'
...
class StripeService {
  ...
  async connectToWebhook(request: HttpRequest) {
  ...
  async createCheckoutSession(user: User, planSlug: string) {
    ...
    const couponConfig = discounts.length ? {} : { allow_promotion_codes: true }
