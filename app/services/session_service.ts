...
export default class SessionService {
  ...
  async check(user: User) {
    ...
    if (log.ipAddress !== this.ipAddress && this.ipAddress) {
      const location = await GetIpLocation.run(this.ipAddress)
  ...
  async getList(user: User, page: number = 1, perPage: number = 15) {
    await this.signOutTimedOut(user)
    ...
    sessions.forEach((session) => {
      session.isCurrentSession = session.token === this.token
    })
