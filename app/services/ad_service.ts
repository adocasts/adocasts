...
export default class AdService {
  ...
  static async getMediumRectangles(limit: number = 3) {
    ...
    if (promises.length < limit) {
      ...
      const ads = await Promise.all(promises).then((results) => results.filter((ad) => !!ad))
  ...
  static async getTotalStats(adIds: number[]) {
    const query = AdvertisementEvent.query().whereIn('advertisementId', adIds)
    const totalImpressions = await query
      .clone()
      .where('typeId', AnalyticTypes.IMPRESSION)
      .count('id')
      .first()
    const totalClicks = await query.clone().where('typeId', AnalyticTypes.CLICK).count('id').first()
    const uniqueImpressions = await query
      .clone()
      .where('typeId', AnalyticTypes.IMPRESSION)
      .countDistinct(['advertisement_id', 'identity'], 'unique_impressions_count')
      .first()
    const uniqueClicks = await query
      .clone()
      .where('typeId', AnalyticTypes.CLICK)
      .countDistinct(['advertisement_id', 'identity'], 'unique_clicks_count')
      .first()
