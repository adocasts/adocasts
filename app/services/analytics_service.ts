...
export default class AnalyticsService {
  static enabled = env.get('PLAUSIBLE_API_KEY') && env.get('PLAUSIBLE_API_KEY') !== '<api_key>'
  private static breakdownEndpoint =
    'https://analytics.adocasts.com/api/v1/stats/breakdown?site_id=adocasts.com'
  private static aggregateEndpoint =
    'https://analytics.adocasts.com/api/v1/stats/aggregate?site_id=adocasts.com'
   ...
   * @returns
   */
  static async getPastMonthsPopularContentSlugs(limit = 8) {
    if (!this.enabled) return []

    return await CacheService.try(
      'ANALYTICS_PAST_MONTH_POPULAR_CONTENT',
      async () => {
        const contentPathPrefix = ['/lessons/', '/news/', '/streams/', '/posts/']
        const results = await this.getPast30dViews()
        return results.reduce((list: any, result: any) => {
          const prefix = contentPathPrefix.find((path) => result.page.startsWith(path))
  ...
  static async getPageViews(
    path: string,
    startDte: DateTime = this.defaultStartDte,
    endDte = DateTime.now()
  ) {
    if (!this.enabled) return 0

    return await CacheService.try(
      `ANALYTICS_PATH_${path}`,
      async () => {
        const start = startDte.toFormat('yyyy-MM-dd')
        const end = endDte.toFormat('yyyy-MM-dd')
        const { data } = await this.apiGet(
          `${this.aggregateEndpoint}&metrics=pageviews&period=custom&date=${start},${end}&filters=event:page%3D%3D${path}`
        )
