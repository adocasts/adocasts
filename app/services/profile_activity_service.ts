...
export default class ProfileActivityService {
  start: DateTime = DateTime.now().startOf('day').minus({ months: 12 })
  ...
  async getPostActivity() {
    const posts = await this.user!.related('posts')
      .query()
  ...
  async getDiscussionActivity() {
    const discussions = await this.user!.related('discussions')
      .query()
  ...
  async getCommentActivity() {
    const comments = await this.user!.related('comments')
      .query()
      .where('createdAt', '>=', this.startSql!)
      .preload('post')
  ...
  async getCompletedLessons() {
    const histories = await this.user!.related('completedPosts')
      .query()
      .where('createdAt', '>=', this.startSql!)
      .preload('post')
  ...
  async getRequestedLessons() {
    const requests = await this.user!.related('lessonRequests')
      .query()
      .where('createdAt', '>=', this.startSql!)
  ...
  async getRequestedLessonUpvotes() {
    const upvotes = await this.user!.related('lessonRequestVotes')
      .query()
      .where('createdAt', '>=', this.startSql!)
