...
export default class VttService {
  ...
  public static parse (input: string, options: VttOptions = {}) {
  ...
  public static parseMeta (headerParts: string[]) {
  ...
  public static parseCues (cues: string[], strict: boolean) {
    const errors: any[] = [];
   ...
   *                       Null if it's a note
   */
  public static parseCue (cue: string, i: number, strict: boolean) {
  ...
  public static validTimestamp (timestamp: string) {
    return this.TIMESTAMP_REGEXP.test(timestamp);
  }
  
  public static parseTimestamp (timestamp: string) {
