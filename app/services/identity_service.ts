...
export default class IdentityService {
  ...
  public static async getLocation(ip: string | undefined) {
    const fallback = { city: undefined, countryLong: undefined, countryShort: undefined }
    if (app.inTest || !ip) return fallback

    try {
      const ip2Location = new IP2Location()
      const binPath = ip.includes(':') 
        ? env.get('IPLOCATION_V6')
        : env.get('IPLOCATION_V4')
  ...
  public static async getRequestIdentity(request: Request) {
    const ip = request.ip()
    const agent = request.header('user-agent')
    return this.create(ip, agent)
  }

  public static async getIdentity(ip: string, agent: string) {
    return this.create(ip, agent)
  }

  public static async getByIdentity(table: string, identity: string, identityKey: string = 'name'): Promise<string> {
  ...
  public static async create(ip: string, agent?: string): Promise<string> {
