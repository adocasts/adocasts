...
export const hexCodeToDecimal = (str: string) => Number.parseInt(str.replace(/^#/, ''), 16)
...
export default class DiscordLogger {
   ...
   */
  log = async (level: LEVELS, data: LogMessage) => {
    try {
   ...
   * @param data Log message data
   */
  error = async (data: LogMessage) => this.log('error', data)

  /**
   * Send a warning log message to discord
   * @param data Log message data
   */
  warn = async (data: LogMessage) => this.log('warn', data)

  /**
   * Send an info log message to discord
   * @param data Log message data
   */
  info = async (data: LogMessage) => this.log('info', data)

  /**
   * Send a verbose log message to discord
   * @param data Log message data
   */
  verbose = async (data: LogMessage) => this.log('verbose', data)

  /**
   * Send a debug log message to discord
   * @param data Log message data
   */
  debug = async (data: LogMessage) => this.log('debug', data)

  /**
   * Send a silly log message to discord
   * @param data Log message data
   */
  silly = async (data: LogMessage) => this.log('silly', data)
}
