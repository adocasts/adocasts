...
export default class FillSessionUaFields extends BaseCommand {
  ...
  async run() {
    ...
    try {
      const updated = []
      const sessions = await SessionLog.query().whereNull('browserName').whereNotNull('userAgent')
    ...
    this.logger.info(
      `Finished "FillSessionUaFields" ${this.exitCode !== 1 ? 'successfully' : 'with errors'}`
    )
  }
}
