...
export default class CleanExpired extends BaseCommand {
  ...
  async run() {
    ...
    this.logger
      .action(`Deleted ${numDeletedSessionLogs} records from ${SessionLog.table}`)
      .succeeded()
    ...
    this.logger
      .action(`Deleted ${numDeletedRateLimits} records from ${RateLimit.table}`)
      .succeeded()
    ...
    this.logger
      .action(`Deleted ${numDeletedAuthAttempts} records from ${AuthAttempt.table}`)
      .succeeded()

    // delete remember me tokens that expired more than 1 month ago
    const [numDeletedRememberMeTokens] = await db
      .from('remember_me_tokens')
      .where('expires_at', '<', DateTime.now().minus({ month: 1 }).toSQL())
      .delete('*')

    this.logger
      .action(`Deleted ${numDeletedRememberMeTokens || 0} records from remember_me_tokens`)
      .succeeded()
